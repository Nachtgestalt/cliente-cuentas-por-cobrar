...
@Component({
  ...
  templateUrl: './history-director.component.html',
  styleUrls: ['./history-director.component.css']
})
export class HistoryDirectorComponent implements OnInit {

  season = JSON.parse(localStorage.getItem('season'));

  displayedColumns = ['fecha', 'cantidad'];
  exampleDatabase: ComisionesService | null;
  dataSource: ComisionDirectorDataSource | null;
...
export class ComisionDirectorDataSource extends DataSource<any> {
  season = JSON.parse(localStorage.getItem('season'));
  ...
  connect(): Observable<any[]> {
    ...
    const displayDataChanges = [
      this._exampleDatabase.dataChangeComisionDirector,
